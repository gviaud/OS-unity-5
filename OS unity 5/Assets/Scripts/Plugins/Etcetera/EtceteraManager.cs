...
public class EtceteraManager : AbstractManager
{
	...
	//indique si le panel de choix de la photo iPhone est affiché.
	public static bool prompt = false;
	
	public static bool getPrompt()
	{
		Debug.Log ("prompt :" + prompt);
		return prompt;
	}
	
	public static void setPrompt(bool value)
	{
		Debug.Log ("prompt :" + prompt);
		prompt = value;
	}
	...
	public void imagePickerDidCancel( string empty )
	{
		...
		else
		{
			...
			setPrompt(false);
			GameObject.Find("MainScene").GetComponent<GUIStart>().showStart(true);
		}
	}
	
	public void imagePickerConfirmShot(string empty)
	{
		if(GameObject.Find("camPivot")!=null)
		{
			if(GameObject.Find("camPivot").GetComponent<gyroControl_v2>()!=null)
			{
				GameObject.Find("camPivot").GetComponent<gyroControl_v2>().enabled = false;
				GameObject.Find("LightPivot").GetComponent<LightConfiguration>().setCompassActive(false);
			}
		...
		Debug.Log ("imagePickerConfirmShot");
	}
